            ...
            try:
                async for evt in sse.send_subscribe(params):
                    ...
                    if isinstance(evt, dict) and evt.get("final") and "status" in evt:
                        break
            except Exception:  # ← dump full traceback
...
def main():
    ...
    parser.add_argument("--log-level",   choices=["debug", "info", "warning", "error"],
                        default="info",  help="Client logging level")
    ...
    p_send.add_argument("--prefix", default=None,
                        help="Handler mount shorthand or full URL "
                             "(e.g. pirate_agent or http://host:8000/chef_agent)")
    p_send.add_argument("text", help="Text of the task to send")
    p_send.add_argument("--wait", action="store_true",
                        help="Wait and stream status/artifacts")
