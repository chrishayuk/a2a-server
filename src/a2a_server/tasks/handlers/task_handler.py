...
class SessionAwareTaskHandler(TaskHandler):
    """
    Enhanced base class for task handlers with comprehensive session management.
    
    This implementation provides a solid foundation for handlers that need
    session support with proper external storage integration.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize session-aware task handler.
        
        Args:
            name: Handler name
            sandbox_id: Sandbox identifier for isolated sessions
            session_sharing: Enable cross-handler session sharing
            shared_sandbox_group: Group for shared sessions
            **kwargs: Additional configuration
        """
        self._name = name
        
        # Determine session sharing configuration
        self.session_sharing = self._determine_session_sharing(session_sharing, shared_sandbox_group)
        self.sandbox_id = sandbox_id or f"handler-{name}"
        self.shared_sandbox_group = shared_sandbox_group
        
        # Log configuration
        if self.session_sharing:
            logger.debug(f"Handler '{name}' configured for SHARED sessions (group: {self.shared_sandbox_group})")
        else:
            logger.debug(f"Handler '{name}' configured for ISOLATED sessions (sandbox: {self.sandbox_id})")
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "session_storage": "external",
            "cross_handler_compatible": self.session_sharing
        }


class SimpleTaskHandler(TaskHandler):
    """
    Simple base class for stateless task handlers.
    
    Use this for handlers that don't need session management
    or complex state tracking.
    """
    
    def __init__(self, name: str, **kwargs):
        """
        Initialize simple task handler.
