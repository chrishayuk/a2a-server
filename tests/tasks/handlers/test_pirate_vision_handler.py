...
#     """Fixture for a message containing an image."""
#     return Message(
#         role=Role.user,
#         parts=[
#             ImagePart(
...
#         assert handler.supports_sessions is False  # Uses base class default
...
#             assert isinstance(events[i].artifact.parts[0], TextPart)
...
#         # Extract the pirate commentary text
#         pirate_lines = [
#             event.artifact.parts[0].text for event in artifact_events
#         ]
...
#         # (The handler doesn't have special cancellation logic, 
#         # but it should handle generator closure gracefully)
...
# class TestPirateVisionIntegration:
#     """Integration tests for PirateVisionHandler."""

#     @pytest.mark.asyncio
#     async def test_full_workflow_simulation(self):
#         """Test complete workflow as it would be used by the server."""
#         handler = PirateVisionHandler()
        
#         # Simulate a complete image analysis request
...
#                 TextPart(
#                     type="text",
#                     text="Ahoy! What be in this here picture?"
#                 )
#             ]
#         )
...
#         all_events = []
#         async for event in handler.process_task("workflow_test", image_message):
...
#         states = [event.status.state for event in all_events 
#                  if isinstance(event, TaskStatusUpdateEvent)]
#         assert states == [TaskState.working, TaskState.completed]
        
#         # Check artifacts contain pirate commentary
#         artifacts = [event.artifact for event in all_events 
#                     if isinstance(event, TaskArtifactUpdateEvent)]
#         assert len(artifacts) == 3
        
#         # Verify each artifact has pirate-themed content
#         for artifact in artifacts:
#             text = artifact.parts[0].text
#             assert any(word in text.lower() for word in [
#                 "arrr", "aye", "treasure", "doubloons", "sailor", "beast", "mane"
#             ])
...
#         try:
#             events = []
#             async for event in handler.process_task("error_test", None):
#                 events.append(event)
#             # If it doesn't crash, that's good
#         except Exception:
#             # Expected to potentially fail with None message
#             pass
        
#         # Test with malformed message parts
#         malformed_message = Message(role=Role.user, parts=None)
#         try:
#             events = []
#             async for event in handler.process_task("malformed_test", malformed_message):
#                 events.append(event)
#             # Should handle gracefully
#         except Exception:
#             # Acceptable to fail with malformed input
#             pass
...
#             role=Role.user,
#             parts=[
#                 TextPart(type="text", text="Test pirate vision")
#             ]
#         )
...
#                 print(f"Artifact {event.artifact.index}: {text}")
